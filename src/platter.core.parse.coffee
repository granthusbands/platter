...
# 	# (1)=inop, (2)=exprop, (3)=rest
# 	inopre = /^\\s*(?:(#{expropnames})|(#{inopnames})(?=[^#{inonly}]|$)|[#{inonly}]+|$)(.*)/
# 	# Find a prefix operator. A set of known things or a sequence of op-like characters
# 	# (1)=preop, (2)=rest, 
# 	preopre = /^\\s*(?:(#{preopnames})(?=[^#{inonly}])|[#{inopspec}]+)(.*)/
# 	# Find an expression value. Basically, anything that comes before a prefix operator
# 	# (1)=ident, (2)=number, (3)=strsing, (4)=strdoub, (5)=other (6)=rest
# 	valre = /^(?:(#{ident})|(#{number})|(#{strsing})|(#{strdoub})|(.*?))(\\s*(?:(#{expropnames})|(#{inopnames})(?=[^#{inonly}]|$)|[#{inonly}]+|$)(.*))/
# """
...
# ? cannot be unwound until : comes along
inops['?'].pri = 101
inops['('].pri = 101
inops['['].pri = 101
